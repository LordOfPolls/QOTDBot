...
from . import utilities, dataclass, monkeypatch
...
slash = monkeypatch.monkeypatched_SlashCommand(bot, sync_commands=False)
...
async def statsUpdate():
    ...
    async with aiohttp.ClientSession(headers=dbHeaders) as session:
        ...
        dBotPayload = {"guilds": len(bot.guilds), "users": totalUsers}
        ...
        if resp.status == 200:
            ...
            log.error(
                f"Failed to update DiscordBotList.com: {resp.status}: {resp.reason}"
            )
            if resp.status == 401:
...
async def on_ready():
    ...
    await bot.change_presence(
        status=discord.Status.online, activity=discord.Game(name="with questions")
    )
...
async def on_guild_join(guild: discord.Guild):
    ...
    try:
        ...
        if (
            ...
            and me.permissions_in(guild.system_channel).send_messages
            and me.permissions_in(guild.system_channel).embed_links
        ):

            embed.description = message.replace("[intro]", f"I am **{bot.user.name}**")
            embed.set_footer(
                text="this message was sent here as this is set as your system channel"
            )
            ...
            for channel in guild.text_channels:
                if (
                    ...
                    and me.permissions_in(channel).send_messages
                    and me.permissions_in(channel).embed_links
                ):
                    embed.description = message.replace(
                        "[intro]", f"I am **{bot.user.name}**"
                    )
                    embed.set_footer(
                        text="this message was sent here as I could not find a system channel, "
        ...
        embed.description = message.replace(
            "[intro]", f"I am **{bot.user.name}**. I was just added to *{guild.name}*"
        )
        embed.set_footer(
...
async def on_guild_remove(guild):
    ...
    try:
        ...
        await bot.db.execute(f"DELETE FROM QOTDBot.guilds WHERE guildID = '{guild.id}'")
